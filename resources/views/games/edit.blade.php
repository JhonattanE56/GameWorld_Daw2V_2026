...
@section('main')
    <div class="row">
        <div class="col-lg-8 mx-auto">
            ...
            <form method="POST"
                  action="{{ route('games.update', $item->id) }}"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="developer" id="developer"
                           ...
                           value="{{ old('developer', $item->developer) }}">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="date_released" id="date_released"
                           class="form-control"
                           value="{{ old('date_released', $item->date_released) }}">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="platform" id="platform" class="form-select" required>
                        ...
                        @foreach ($platforms as $value => $label)
                            <option value="{{ $value }}"
                                @selected(old('platform', $item->platform->value ?? $item->platform) == $value)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="genre" id="genre" class="form-select" required>
                        ...
                        @foreach ($genres as $value => $label)
                            <option value="{{ $value }}"
                                @selected(old('genre', $item->genre->value ?? $item->genre) == $value)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="gamemode" id="gamemode" class="form-select" required>
                        ...
                        @foreach ($gamemodes as $value => $label)
                            <option value="{{ $value }}"
                                @selected(old('gamemode', $item->gamemode->value ?? $item->gamemode) == $value)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="age_rating" id="age_rating" class="form-select">
                        ...
                        @foreach ($age_ratings as $value => $label)
                            <option value="{{ $value }}"
                                @selected(old('age_rating', $item->age_rating->value ?? $item->age_rating) == $value)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" id="description"
                              class="form-control" rows="4">{{ old('description', $item->description) }}</textarea>
                </div>
                    ...
                    @if ($item->image)
                        <div class="mt-2">
                            <img src="{{ asset('storage/' . $item->image) }}"
                                 alt="{{ $item->title }}" class="img-thumbnail" style="max-height: 150px;">
                        </div>
                    @endif
                </div>

                <div class="d-flex justify-content-between mt-4">
                    <a href="{{ route('games.index') }}" class="btn btn-outline-secondary">Volver al catálogo</a>
